...
public class MenuAnimation<T> implements Menu<T> {
    ...
    private String name;
    private Map<String, Boolean> mapVals = new HashMap<String, Boolean>();
    ...
    public void doOneFrame(DrawSurface d, double dt) {
        ...
        for (int i = 0; i < this.returnVals.size(); i++) {
            if (this.keyboard.isPressed(this.choice.get(i)) && !this.mapVals.get(this.choice.get(i))) {
                if (!subMenu.get(i)) {
                    //clicked(this.choice.get(i));
                    this.status = this.returnVals.get(i);
                } else if (subMenu.get(i)) {
                    ...
                    sub.isClicked(this.choice.get(i));
                    ar.run(sub);
                ...
                this.stop = true;
                break;
            } else if (!this.keyboard.isPressed(this.choice.get(i))) {
                this.mapVals.replace(this.choice.get(i), false);
            }
        }
    }
    @Override
    ...
    public void isClicked(String s) {
        if (this.mapVals.get(s) != null) {
            this.mapVals.replace(s, true);
        }
    }
}
